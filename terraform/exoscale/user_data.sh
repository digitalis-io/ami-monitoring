...
mount_data() {
    ...
    local devices
    devices=$(lsblk -dpno NAME | grep -E '^/dev/(sd|xvd|nvme|vd)' || true)
...
export CLOUD_ENVIRONMENT=exoscale
export ANSIBLE_ROLE=role_${role}
export ANSIBLE_USER=ansible
export ANSIBLE_INVENTORY="/opt/ansible-monitoring/inventory/exoscale.yaml"
export ANSIBLE_STACK=stack_${stack}
END

cat > /etc/default/wizard <<END
CLOUD_ENVIRONMENT=exoscale
ANSIBLE_ROLE=role_${role}
ANSIBLE_STACK=stack_${stack}
ANSIBLE_USER=ansible
ANSIBLE_INVENTORY="/opt/ansible-monitoring/inventory/exoscale.yaml"
SOS_ENDPOINT=${sos_endpoint}
END
...
systemctl restart wizard || /bin/true
